...
Camera::Camera(const CameraCI &ci)
{
	position = ci.start_position;
	world_up = ci.start_up;
	yaw      = ci.start_yaw;
	pitch    = ci.start_pitch;
	...
	movement_speed = ci.start_move_speed;
	turn_speed     = ci.start_turn_speed;
...
glm::vec3 Camera::get_front_axis()
{
	return front;
}
...
glm::quat Camera::calculate_viewQuat()
{
