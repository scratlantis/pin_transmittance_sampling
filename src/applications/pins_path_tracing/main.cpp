...
int main()
{
	...
	envMap.createImageView(&gState.heap, envMapImageViewCreateInfo);
	VkSamplerCreateInfo                   envMapSamplerCI       = SamplerCreateInfo_Default(0.0);
	Sampler                               envMapSampler         = Sampler(&gState.cache, envMapSamplerCI);
	std::string                           envMapName            = std::string("autumn_field_2k.hdr");
	//envMapName                                                  = std::string("fluid_sim.jpg");
	...
	while (!gState.io.shouldTerminate())
	{
		...
		cmdBuf.pushDescriptors(0, ubo, (Image) offscreenImage, pinTransmittanceBuf, pinGridBuf, pinGridIdBuf, gaussiansBuf, envMapSampler, envMap);
		cmdBuf.dispatch(workGroupCount);
