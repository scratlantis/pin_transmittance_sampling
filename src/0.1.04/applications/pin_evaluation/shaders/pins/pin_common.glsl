...
void cell_range(uint idx, out vec3 posMin, out vec3 posMax, out vec2 angleMin, out vec2 angleMax)
{
	...
	float thetaMin = PI*0.5 - asin(clamp(dirGridCoords.y + 1.0/float(PIN_DIR_GRID_SIZE),0.0,0.9999) );
...
uint pin_cache_offset(vec3 pos, vec3 dir)
{
	...
	return posOffset * PIN_DIR_GRID_SIZE_PHI * PIN_DIR_GRID_SIZE_THETA + dirOffset;
}
