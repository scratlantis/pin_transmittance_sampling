...
void ComparativePathTracer::render(CmdBuffer cmdBuf, const RenderInfo &renderInfo, Buffer fluxBuffer)
{
	...
	// Render
	if (!gvar_screen_cursor_enable.val.v_bool || gvar_path_sampling_event.val.v_bool || gvar_continuous_path_sampling.val.v_bool)
	{
	...
	if (timeQueryFinished)
	{
		...
		pStrategieA->trace(cmdBuf, renderInfo, outputA);
		...
		pStrategieB->trace(cmdBuf, renderInfo, outputB);
