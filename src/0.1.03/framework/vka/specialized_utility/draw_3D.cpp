...
void cmdShowTriangles(CmdBuffer cmdBuf, IResourcePool *pPool, Image dst, ModelData model, Camera *cam, float scale, VkImageLayout dstLayout)
{
	...
	glm::mat4 vp                                    = projectionMat * cam->getViewMatrix() * glm::scale(glm::mat4(1.0), glm::vec3(scale));
